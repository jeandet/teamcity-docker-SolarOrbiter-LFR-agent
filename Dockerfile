...
RUN dnf install -y git cppcheck bzip2 hg automake autoconf gcc glibc.i686 zlib.i686 ncurses-compat-libs.i686
...
RUN cd /opt && tar -xf /opt/sparc-rtems-4.10-gcc-4.4.6-1.2.20c-linux.tar.bz2 && \
    cd /opt/rtems-4.10/src && tar -xf /opt/rtems-4.10-1.2.20c-src.tar.bz2 && \
    ...
    make compile-drvmgr && \
    make install-drvmgr && \
    rm -f /opt/sparc-rtems-4.10-gcc-4.4.6-1.2.20c-linux.tar.bz2 /opt/rtems-4.10-1.2.20c-src.tar.bz2
